...
```bash
docker build -t mmgo:0.1.0-alpha.8 .
docker run --rm --env-file /secure/path/mmgo.env mmgo:0.1.0-alpha.8 bin/migrate
docker run --rm --env-file /secure/path/mmgo.env mmgo:0.1.0-alpha.8 bin/seed
docker run --name mmgo --env-file /secure/path/mmgo.env -p 4000:4000 mmgo:0.1.0-alpha.8
```
